...
def main():
    parser = argparse.ArgumentParser(description='Test model on training data')
    ...
    for initial_control, initial_state, true_control, true_state, file_name \
        ...
        pred_target, whitebox, blackbox = model.simulate(initial_control, initial_state, true_control, return_whitebox_blackbox=True)
    ...
    result_file_path = os.path.join(result_directory, f'train-w_{window_size}-h_{horizon_size}.hdf5')
