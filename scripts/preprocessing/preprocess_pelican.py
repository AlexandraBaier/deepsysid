...
def main():
    ...
    simulation_names = [fn for fn in os.listdir(raw_dir) if os.path.isfile(os.path.join(raw_dir, fn))]
    sim_df = dict()
    ...
    for simulation_name in train_simulations:
        sim_df[simulation_name].to_csv(os.path.join(processed_dir, 'train', simulation_name), index=False)
    # use data without sensor noise for validation
    for simulation_name in validation_simulations:
        sim_df[simulation_name].to_csv(os.path.join(processed_dir, 'validation', simulation_name), index=False)
    # but use data without sensor noise for testing
    for simulation_name in test_simulations:
        sim_df[simulation_name].to_csv(os.path.join(processed_dir, 'test', simulation_name), index=False)
