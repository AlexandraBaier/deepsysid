...
def main():
    ...
    for simulation_name in simulation_names:
        measured_df = pd.read_csv(os.path.join(interim_dir, simulation_name, 'measurements.csv'))
        ...
        true_df = pd.read_csv(os.path.join(interim_dir, simulation_name, 'true_measurements.csv'))
    ...
    validation_simulations = simulation_names[train_count:train_count+validation_count]
    ...
    for simulation_name in train_simulations:
        measured_sim_df[simulation_name].to_csv(os.path.join(processed_dir, 'train', simulation_name + '.csv'), index=False)
    # use data without sensor noise for validation
    for simulation_name in validation_simulations:
        true_sim_df[simulation_name].to_csv(os.path.join(processed_dir, 'validation', simulation_name + '.csv'), index=False)
    # but use data without sensor noise for testing
    for simulation_name in test_simulations:
        true_sim_df[simulation_name].to_csv(os.path.join(processed_dir, 'test', simulation_name + '.csv'), index=False)
