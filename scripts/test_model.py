...
def main():
    ...
    parser.add_argument(
        '--mode', action='store', help='either "train" or "validation" or "test"'
    )
    ...
    if mode not in {'train', 'validation', 'test'}:
        raise ValueError(
            'Argument to --mode must be either "train", "validation" or "test"'
        )
    ...
    result_file_path = os.path.join(
        result_directory, f'{mode}-w_{config.window_size}-h_{config.horizon_size}.hdf5'
    )
    with h5py.File(result_file_path, 'w') as f:
        ...
        if (len(whiteboxes) > 0) and (len(blackboxes) > 0):
            ...
            for i, (control, pred_state, true_state) in enumerate(
